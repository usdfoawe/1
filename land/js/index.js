...
$(function(){
	...
	// Конечная дата. Месяц начинается с 0
	var time_end = Math.round(((new Date(2014, 5, 5)).getTime()) / 1000);
	
	var time_current = Math.round((new Date()).getTime() / 1000);
	
	var time_interval = time_end - time_current;
	...
	$('.order_form input[type="submit"]').click(function(){
		...
		var no_errors = true;
		var name = $('input[name="name"]', parent).val().toString();
		var phone = $('input[name="phone"]', parent).val().toString();
		
		if (!name.match(/^[A-Za-z А-Яа-я0-9]+$/)) {
			$('input[name="name"]', parent).after('<div class="error">Неправильно указано имя</div>');
			no_errors = false;
		}
		
		if (!phone.match(/^[+\- 0-9()]{5,20}$/)) {
			$('input[name="phone"]', parent).after('<div class="error">Неправильно указан телефон</div>');
			no_errors = false;
		}
		
		$('.error', parent).hide().slideDown();
	...
	$('.slider .slider_buttons').on('jcarouselpagination:active', 'span', function() {
...
function formatTime(seconds) {
	...
	minutes %= 60;
	seconds %= 60;
	html = '<span><b>' + one(hours) + two(hours) + '</b>часов</span> <span><b>' + one(minutes) + two(minutes) + '</b>минут</span> <span><b>' + one(seconds) + two(seconds) + '</b>секунд</span>';
	return html;
};
...
(function dayTimer() {
        ...
        $('.d_hours').text(dayHours);
        $('.d_minutes').text(dayMinutes);
        $('.d_seconds').text(daySeconds);

        setTimeout(dayTimer, 1000);
})();
